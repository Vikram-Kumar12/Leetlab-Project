...
const RightSidePage = () => {
  ...
  const [statusAndLanguage, setStatusAndLanguage] = useState(null);
  ...
  useEffect(() => {
    if (submissions.length > 0) {
      // find status of all submission by login user
      const status_language = submissions.map((elem) => ({
        status: elem.status,
        language: elem.language,
      }));
      setStatusAndLanguage(status_language);

      // count accepted status of login user
      let acceptedStatus = 0;
      let javaScript = 0;
      let python = 0;
      let java = 0;
      status_language.forEach((elem) => {
        if (elem.status === "Accepted") {
          acceptedStatus += 1;
          if (elem.language === "JavaScript") javaScript += 1;
          if (elem.language === "Python") python += 1;
          if (elem.language === "Java") java += 1;
        }
      });
      setSubmissionCount(acceptedStatus);
      setLanguage({ javaScript, python, java });
    }
  }, [submissions]);
  // console.log("statusAndLanguage", statusAndLanguage);
            ...
            >
              {[
                ...
                },
                {
                  ...
                  count: `${difficultyCounts.mediumCount}`,
                },
                {
                  label: "Hard",
                  color: "text-red-500",
                  count: `${difficultyCounts.hardCount}`,
                },
              ].map((item, i) => (
                <div
                  key={i}
